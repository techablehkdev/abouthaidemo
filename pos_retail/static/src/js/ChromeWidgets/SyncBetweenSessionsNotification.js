        ...
        async onClick() {
            if (!this.env.pos.config.sync_multi_session_manual_stop) {
                return this.env.pos.alert_message({
                    title: this.env._t('Sync Between Session Worked !'),
            ...
            if (this.env.pos.config.sync_multi_session) {
                this.showPopup('ConfirmPopup', {
                    ...
                    body: this.env._t('Sync Working back now .'),
                    disableCancelButton: true,
                })
            } else {
                this.showPopup('ErrorPopup', {
                    title: this.env._t('Warning'),
                    body: this.env._t('Stop Sync Between Session. If you need working back, please on more click to Icon'),
                    disableCancelButton: true,
                })
            }
        }
