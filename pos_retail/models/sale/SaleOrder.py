...
class sale_order(models.Model):
    ...
    qr_items = fields.Char(compute='_compute_qr_items')

    def _compute_qr_items(self):
        for order in self:
            order.qr_items = ""
            for line in order.order_line:
                order.qr_items += "%s:%s:%s:%s:N/A;" % (line.product_id.id, line.product_uom_qty,
                                                    line.discount or "N/A", line.price_unit)
    ...
    def search_read(self, domain=None, fields=None, offset=0, limit=None, order=None):
        ...
        if context.get('pos_config_id', None):
            ...
            if config.load_booked_orders_limit > 0:
                limit = config.load_booked_orders_limit
            else:
                limit = 100
            order = 'id DESC'
