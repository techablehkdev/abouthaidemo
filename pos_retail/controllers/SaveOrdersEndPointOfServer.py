    ...
    def __init__(self):
        ...
        for session in sessions:
            request.env['bus.bus']._sendone(
                session.user_id.partner_id, 'pos.test.polling', session.user_id.partner_id.id)
    ...
    def push_orders(self, message, config_id):
        ...
        if self.auto_push_orders:
            ...
            for order in orders:
                ...
                if (order.amount_paid > 0 and order.amount_total > 0 and order.amount_paid >= order.amount_total) or (
                        is_paid):
